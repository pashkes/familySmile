...
!function (e, t) {
  ...
  "undefined" != typeof document && function (e) {
    ...
    function a(t) {
      "html" !== t.target.nodeName.toLowerCase() && (e = !1, document.removeEventListener("mousemove", a), document.removeEventListener("mousedown", a), document.removeEventListener("mouseup", a), document.removeEventListener("pointermove", a), document.removeEventListener("pointerdown", a), document.removeEventListener("pointerup", a), document.removeEventListener("touchmove", a), document.removeEventListener("touchstart", a), document.removeEventListener("touchend", a));
    }

    document.addEventListener("keydown", function (t) {
      d(document.activeElement) && i(document.activeElement), e = !0;
    }, !0), document.addEventListener("mousedown", s, !0), document.addEventListener("pointerdown", s, !0), document.addEventListener("touchstart", s, !0), document.addEventListener("focus", function (t) {
      var n, s, u;
      d(t.target) && (e || (n = t.target, s = n.type, "INPUT" == (u = n.tagName) && o[s] && !n.readOnly || "TEXTAREA" == u && !n.readOnly || n.isContentEditable)) && i(t.target);
    }, !0), document.addEventListener("blur", function (e) {
      var o;
      d(e.target) && (e.target.classList.contains("focus-visible") || e.target.hasAttribute("data-focus-visible-added")) && (t = !0, window.clearTimeout(n), n = window.setTimeout(function () {
...
var tns = function () {
  ...
  var tns = function tns(options) {
    ...
    function sliderInit() {
      ...
      initStructure(); // add id, class, aria attributes
      // before clone slides
    ...
    var updateIndex = function () {
      ...
      function () {
        var leftEdge = indexMin,
            rightEdge = indexMax;
        leftEdge += slideBy;
        rightEdge -= slideBy; // adjust edges when has edge padding
        ...
        for (; i < len; i++) {
          forEachNodeList(slideItems[i].querySelectorAll(lazyloadSelector), function (img) {
            if (!hasClass(img, imgCompleteClass)) {
              ...
              addEvents(img, eve);
              addEvents(img, imgEvents); // update srcset
...
(function (root, factory) {
...
})(typeof global !== 'undefined' ? global : window || (void 0).global, function (root) {
  'use strict'; //
  // Variables
  //
  ...
  var settings, tab; // Default settings
  ...
  var stopVideos = function stopVideos(content, settings) {
    ...
    if (!settings.stopVideo) return; // Only run if content container is closed
  ...
  var adjustFocus = function adjustFocus(tab, settings) {
    if (tab.hasAttribute('data-tab-no-focus')) return; // If tab is closed, remove tabindex
  ...
  var toggleToggles = function toggleToggles(toggle, settings) {
    // Variables
    ...
    var toggleList; // Show or hide each toggle
    // @todo Start here
    ...
    forEach(toggles, function (item) {
      ...
      if (item.hash === toggle.hash) {
        // Add active class
        item.classList.add(settings.toggleActiveClass); // If toggle is a list item, activate <li> element, too
  ...
  var toggleTabs = function toggleTabs(tabID, settings) {
    // Variables
  ...
  var hashChangeHandler = function hashChangeHandler(event) {
    // Get hash from URL
    var hash = root.location.hash; // If clicked tab is cached, reset it's ID
  ...
  var clickHandler = function clickHandler(event) {
    ...
    if (event.button !== 0 || event.metaKey || event.ctrlKey) return; // Check if event target is a tab toggle

    var toggle = getClosest(event.target, settings.selectorToggle);
    if (!toggle || !toggle.hash) return; // Don't run if toggle points to currently open tab
    ...
    tab = document.querySelector(toggle.hash); // If tab content exists, save the ID as a data attribute and remove it (prevents scroll jump)
  ...
  var focusHandler = function focusHandler(event) {
    ...
    if (!tab) return; // Don't run if the content area is already open

    if (tab.classList.contains(settings.contentActiveClass)) return; // Store tab ID to variable and remove it from the tab

    var hash = tab.id;
    tab.setAttribute('data-tab-id', hash);
    tab.setAttribute('data-tab-no-focus', true);
    tab.id = ''; // Change the hash
  ...
  tabby.init = function (options) {
    ...
    tabby.destroy(); // Merge user options with defaults
    ...
    root.addEventListener('hashchange', hashChangeHandler, false); // If URL has a hash, activate hashed tab by default
 ...
 */
var CustomSelect = function CustomSelect(e) {
  var l = "string" == typeof e.elem ? document.getElementById(e.elem) : e.elem,
      s = "js-Dropdown-title",
      o = "is-selected",
      t = "is-open",
      n = l.querySelectorAll("option"),
      i = n.length,
      r = document.createElement("div");
  r.className = "js-Dropdown", l.id && (r.id = "custom-" + l.id);
  var a = document.createElement("button");
  a.className = s, a.textContent = n[0].textContent;
  var c = document.createElement("ul");
  c.className = "js-Dropdown-list";

  for (var d = 0; d < i; d++) {
    var u = document.createElement("li");
    u.innerText = n[d].textContent, u.setAttribute("data-value", n[d].value), u.setAttribute("data-index", d), null !== n[d].getAttribute("selected") && (u.classList.add(o), a.textContent = n[d].textContent), c.appendChild(u);
  }
  ...
  return r.appendChild(a), r.appendChild(c), r.addEventListener("click", function (e) {
    e.preventDefault();
    var t = e.target;
    t.className === s && m();

    if ("LI" === t.tagName) {
      r.querySelector("." + s).innerText = t.innerText, l.options.selectedIndex = t.getAttribute("data-index");
      var n = new Event("change");
      l.dispatchEvent(n);

      for (var a = 0; a < i; a++) {
        c.querySelectorAll("li")[a].classList.remove(o);
      }

      t.classList.add(o), v();
    }
  }), l.parentNode.insertBefore(r, l), l.style.display = "none", document.addEventListener("click", function (e) {
    r.contains(e.target) || v();
  }), {
    toggle: m,
    close: v,
    open: function open() {
...
(function () {
  ...
  window.onload = function () {
    var buttonsGroup = Array.from(document.querySelectorAll('.js-button-group'));
    buttonsGroup.forEach(function (element) {
      element.addEventListener('click', function (evt) {
        buttonsGroup.forEach(function (currentButtons) {
          return currentButtons.classList.remove('active');
        });
...
(function () {
  ...
  var btnToSelect = document.querySelector('.js-simulate-click-to-select');
...
(function () {
  ...
  var close = menuWrapper.querySelector('.js-menu-close');
  menuButton.addEventListener('click', function (evt) {
    var target = evt.target.closest('.js-show-menu');
  ...
  var menuAnchor = Array.from(document.querySelectorAll('.anchor'));
  menuAnchor.forEach(function (element) {
    element.addEventListener('click', function () {
      menuWrapper.classList.remove('opened');
      menuButton.setAttribute('aria-expanded', 'false');
    });
  });
  var requestBtn = document.querySelector('.js-modal-request');
  requestBtn.addEventListener('click', function () {
    var form = document.querySelector('.request-modal .js-request-form');
    ...
    form.addEventListener('submit', function (e) {
      e.preventDefault(); // if form success

      var closeModal = document.querySelector('.js-modal-close');
      closeModal.click();
      var success = document.querySelector('.js-request-success');
      success.click();
    });
  });
})();
...
(function () {
  ...
  form.addEventListener('submit', function (e) {
    ...
    var success = document.querySelector('.js-order-success');
    success.click();
  });
})();
"use strict";
...
(function () {
  var select = document.querySelector('.js-select-services');
  if (!select) return;
...
(function () {
  var showMore = Array.from(document.querySelectorAll('.js-expandmore'));
  var expandBlock = document.querySelectorAll('.js-expand-more ');
  showMore.forEach(function (element, index) {
    element.addEventListener('click', function (evt) {
      ...
      var hasOtherExpandBlock = evt.target.closest('.js-expandmore').previousSibling.parentElement.querySelector('.js-to_expand ');
